...
Route::prefix('api/app')->group(function () {
    ...
    Route::middleware('auth:api')->group(function () {
        ...
        Route::prefix('driver')->group(function () {
            ...
            // Driver App Compatibility Aliases
            Route::get('requests', [AppDriverController::class, 'getRideRequests']);
            Route::get('ride/active', [AppDriverController::class, 'getActiveRide']);
            Route::post('location', [AppAuthController::class, 'saveLocation']);
            
            Route::post('ride/{id}/decline', function ($id) {
                request()->merge(['status' => 'cancelled']);
                return app(AppDriverController::class)->updateRideStatus(request(), $id);
            });
            
            Route::post('ride/{id}/arrived', function ($id) {
                request()->merge(['status' => 'arrived']);
                return app(AppDriverController::class)->updateRideStatus(request(), $id);
            });
            
            Route::post('ride/{id}/start', function ($id) {
                request()->merge(['status' => 'in_progress']);
                return app(AppDriverController::class)->updateRideStatus(request(), $id);
            });
            
            Route::post('ride/{id}/bid', function ($id) {
