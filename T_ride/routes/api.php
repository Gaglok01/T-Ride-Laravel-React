...
Route::middleware('auth:api')->group(function () {
    Route::prefix('admin')->middleware('role:admin')->group(function () {
        ...
        // Dispatch Management
        Route::get('/dispatch/zones', [DispatchController::class, 'getZones']);
        Route::get('/dispatch/commission-settings', [DispatchController::class, 'getCommissionSettings']);
        Route::post('/dispatch/commission-settings', [DispatchController::class, 'updateCommissionSettings']);
        Route::get('/dispatch/settings', [DispatchController::class, 'getSettings']);
        Route::post('/dispatch/settings', [DispatchController::class, 'updateSettings']);
        Route::post('/dispatch/emergency-stop', [DispatchController::class, 'emergencyStop']);
        ...
        Route::post('/dispatch/auto-assign', [DispatchController::class, 'autoAssign']);
        Route::post('/dispatch/attempts/{attempt}/reject', [DispatchController::class, 'markAttemptRejected']);
        Route::post('/dispatch/attempts/{attempt}/timeout', [DispatchController::class, 'markAttemptTimeout']);
        Route::post('/dispatch/attempts/{attempt}/accept', [DispatchController::class, 'markAttemptAccepted']);
...
Route::post('/app/driver/ride/{id}/accept', function (\Illuminate\Http\Request $request, $id) {
    ...
    Route::post('/app/driver/ride/{id}/decline', function (\Illuminate\Http\Request $request, $id) {
        $request->merge(['action' => 'reject']);
        return app(\App\Http\Controllers\Api\AppDriverController::class)
            ->respondToRide($request, $id);
    });

    Route::post('/app/driver/ride/{id}/arrived', function (\Illuminate\Http\Request $request, $id) {
        $request->merge(['status' => 'arrived']);
        return app(\App\Http\Controllers\Api\AppDriverController::class)
            ->updateRideStatus($request, $id);
    });

    Route::post('/app/driver/ride/{id}/start', function (\Illuminate\Http\Request $request, $id) {
        $request->merge(['status' => 'started']);
        return app(\App\Http\Controllers\Api\AppDriverController::class)
            ->updateRideStatus($request, $id);
    });

    Route::post('/app/driver/ride/{id}/complete', function (\Illuminate\Http\Request $request, $id) {
        $request->merge(['status' => 'completed']);
        return app(\App\Http\Controllers\Api\AppDriverController::class)
            ->updateRideStatus($request, $id);
    });

    Route::post('/app/driver/ride/{id}/bid', function (\Illuminate\Http\Request $request, $id) {
