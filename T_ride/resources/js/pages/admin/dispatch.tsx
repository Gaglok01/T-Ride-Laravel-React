...
import { GoogleMap, Marker, Autocomplete, DirectionsRenderer, Circle } from "@react-google-maps/api"
...
export default function DispatchPage() {
    ...
    const [heatZones, setHeatZones] = useState<any[]>([])
    const [dispatchZones, setDispatchZones] = useState<any[]>([])
    ...
    const center = {
        lat: 41.2565, // Default fallback Omaha
        lng: -95.9345
    }
    ...
    const fetchAllData = async () => {
        ...
        try {
            const heatmapResponse = await axios.get('/public/driver/heat-map')
            if (heatmapResponse.data?.status) {
                const zones = heatmapResponse.data.data || []
                setHeatZones(zones)
                ...
                if (zones.length > 0) {
                    ...
                    if (dispatchMapRef.current) {
                        // // dispatchMapRef.current.fitBounds(bounds)
                    } else {
                        setCenter({
                            lat: Number(zones[0].lat),
                            lng: Number(zones[0].lng),
                        })
                    }
                }
            }
            ...
            const results = await Promise.allSettled([
                ...
                axios.get('/admin/dispatch/settings'),
                axios.get('/admin/dispatch/zones'),
                axios.get('/admin/dispatch/commission-settings')
            ])

            const [statsRes, ordersRes, activeRes, driversRes, settingsRes, zonesRes, commissionRes] = results
            ...
            if (settingsRes?.status === 'fulfilled' && settingsRes.value.data.status) {
                setDispatchSettings(settingsRes.value.data.data)
            }

            if (zonesRes?.status === 'fulfilled' && zonesRes.value.data.status) {
                setDispatchZones(zonesRes.value.data.data)
            }

            if (commissionRes?.status === 'fulfilled' && commissionRes.value.data.status) {
                setCommissionSettings(commissionRes.value.data.data)
            }
        } catch (error) {
    ...
    const showOrderRoute = (order: any) => {
        if (!window.google || !availableDrivers.length) return

        const pickupLat = Number(order.pickup_lat ?? order.pickup_latitude ?? order.lat)
        const pickupLng = Number(order.pickup_lng ?? order.pickup_longitude ?? order.lng)
        const dropoffLat = Number(order.dropoff_lat ?? order.dropoff_latitude)
        const dropoffLng = Number(order.dropoff_lng ?? order.dropoff_longitude)

        if (!Number.isFinite(pickupLat) || !Number.isFinite(pickupLng) || !Number.isFinite(dropoffLat) || !Number.isFinite(dropoffLng)) return

        const nearestDriver = availableDrivers
            .map((driver: any) => {
                const lat = Number(driver.current_lat ?? driver.lat ?? driver.latitude)
                const lng = Number(driver.current_lng ?? driver.lng ?? driver.longitude)
                const score = Math.abs(lat - pickupLat) + Math.abs(lng - pickupLng)
                return { ...driver, lat, lng, score }
            })
        ...
        new google.maps.DirectionsService().route(
            {
                origin: { lat: nearestDriver.lat, lng: nearestDriver.lng },
                destination: { lat: dropoffLat, lng: dropoffLng },
                waypoints: [{ location: { lat: pickupLat, lng: pickupLng }, stopover: true }],
            ...
            (result, status) => {
                if (status === google.maps.DirectionsStatus.OK && result) {
                    ...
                    setRouteInfo({
                        ...
                        toPickupEta: legs[0]?.duration?.text || 'N/A',
                        toPickupDistance: legs[0]?.distance?.text || 'N/A',
                        tripEta: legs[1]?.duration?.text || 'N/A',
                        tripDistance: legs[1]?.distance?.text || 'N/A',
                    })
                }
            }
        )
    ...
    const updateDispatchSetting = (key: string, value: any) => {
        setDispatchSettings((prev: any) => ({ ...(prev || {}), [key]: value }))
    }
    ...
    const saveDispatchSettings = async () => {
        ...
        try {
            const res = await axios.post('/admin/dispatch/settings', dispatchSettings)
            if (res.data?.status) {
                setDispatchSettings(res.data.data)
            }
        } catch (error) {
            console.error('Failed to save dispatch settings:', error)
            window.alert('Failed to save dispatch settings.')
        } finally {
            setSavingSettings(false)
        }
    }

    const updateCommissionSetting = (serviceType: string, value: number) => {
        setCommissionSettings((prev: any[]) =>
            prev.map((item: any) =>
                item.service_type === serviceType
                    ? { ...item, commission_percent: value }
                    : item
            )
        )
    }
    ...
    const saveCommissionSettings = async () => {
        ...
        try {
            const res = await axios.post('/admin/dispatch/commission-settings', {
                commissions: commissionSettings,
            })
            if (res.data?.status) setCommissionSettings(res.data.data)
        } catch (error) {
                    ...
                    <div className="bg-tride-hover rounded-2xl flex-1 overflow-hidden relative">
                        {isLoaded ? (
                            <GoogleMap
                                ...
                                center={center}
                                onLoad={(map) => { dispatchMapRef.current = map }}
                                zoom={12}
                                options={{
                            ...
                            >
                                ...
                                {routeDirections && (
                                    <>
                                        <DirectionsRenderer directions={routeDirections} options={{ preserveViewport: false }} />
                                        <div className="absolute top-4 right-4 z-20">
                                            <Button
                                                variant="secondary"
                                                size="sm"
                                                className="bg-tride-card/90 backdrop-blur border border-tride-border"
                                                onClick={() => {
                                ...
                                {dispatchZones.map((zone: any) => (
                                    <Circle
                                        ...
                                        options={{
                                            fillColor: zone.heat_level === 'high' ? '#ef4444' : zone.heat_level === 'medium' ? '#f59e0b' : '#22c55e',
                                            fillOpacity: zone.heat_level === 'high' ? 0.18 : 0.10,
                                            strokeColor: zone.heat_level === 'high' ? '#ef4444' : zone.heat_level === 'medium' ? '#f59e0b' : '#22c55e',
                                            strokeOpacity: 0.65,
                                            strokeWeight: 2,
                                            clickable: false,
                                        }}
                                ...
                                {availableDrivers
                                    ...
                                    .map((driver: any, index: number) => {
                                        ...
                                        const driverStatus = String(driver.status ?? driver.availability_status ?? 'available').toLowerCase()
                                        const driverIcon =
                                            driverStatus.includes('busy') || driverStatus.includes('trip') || driverStatus.includes('ride')
                                                ? "https://maps.google.com/mapfiles/ms/icons/blue-dot.png"
                                                : driverStatus.includes('offline') || driverStatus.includes('inactive')
                                        ...
                                        return (
                                            <Marker
                                                ...
                                                position={{
                                                    lat: lat + ((index + 1) * 0.00012),
                                                    lng: lng + ((index + 1) * 0.00012),
                                                }}
                                                icon={{ url: driverIcon }}
                                                title={`DRIVER: ${driver.name} • ${driverStatus}`}
                                            />
                                        )
                                    })}
                                ...
                                {pendingOrders
                                    .filter((order: any) => {
                                        const lat = Number(order.pickup_lat ?? order.pickup_latitude ?? order.lat)
                                        const lng = Number(order.pickup_lng ?? order.pickup_longitude ?? order.lng)
                                        return Number.isFinite(lat) && Number.isFinite(lng) && lat !== 0 && lng !== 0
                                    })
                                    .map((order: any, index: number) => {
                                        ...
                                        return (
                                            <Marker
                                                key={`pending-${order.source_type}-${order.source_id}`}
                                                onClick={() => showOrderRoute(order)}
                                                position={{
                                                    lat: lat - ((index + 1) * 0.00012),
                                                    lng: lng - ((index + 1) * 0.00012),
                                                }}
                        ...
                        {routeInfo && (
                            <div className="absolute top-4 left-4 bg-tride-card/95 backdrop-blur border border-tride-border p-4 rounded-2xl text-xs shadow-xl w-[280px] z-20">
                                <div className="flex items-start justify-between mb-3">
                                    <div>
                                        <h4 className="font-bold text-tride-text">Dispatch Intelligence</h4>
                                        <p className="text-tride-text-muted">Order {routeInfo.orderId}</p>
                                    </div>
                                    <button
                                        className="text-tride-text-muted hover:text-white"
                                        onClick={() => {
                                            setRouteInfo(null)
                                            setRouteDirections(null)
                                        }}
                                ...
                                <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 mb-2">
                                    <p className="text-emerald-400 font-semibold">Nearest Driver</p>
                                    <p className="text-tride-text font-bold">{routeInfo.driverName}</p>
                                </div>

                                <div className="grid grid-cols-2 gap-2">
                                    <div className="p-3 rounded-xl bg-blue-500/10 border border-blue-500/20">
                                        <p className="text-blue-400 font-semibold">To Pickup</p>
                                        <p className="text-tride-text font-bold">{routeInfo.toPickupEta}</p>
                                        <p className="text-tride-text-muted">{routeInfo.toPickupDistance}</p>
                                    </div>

                                    <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
                                        <p className="text-amber-400 font-semibold">Trip</p>
                                        <p className="text-tride-text font-bold">{routeInfo.tripEta}</p>
                                        <p className="text-tride-text-muted">{routeInfo.tripDistance}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                                ...
                                <tbody>
                                    {loading ? (
                                        ...
                                        pendingOrders.map(order => (
                                            <tr key={`${order.source_type}-${order.source_id}`} onClick={() => showOrderRoute(order)} className="border-b border-tride-border/50 hover:bg-tride-hover/30 transition-colors group cursor-pointer">
                                                ...
                                                <td className="px-4 py-4 text-right">
                                                    <div className="flex justify-end gap-2">
                                                        <Button variant="secondary" size="sm" className="h-7 text-xs" onClick={(e) => { e.stopPropagation(); showOrderRoute(order) }}>Route</Button>
                ...
                {activeTab === "AI Routing Config" && (
                    <div className="bg-tride-card border border-tride-border rounded-3xl p-6">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-lg font-semibold text-tride-text">AI Routing Config</h3>
                                <p className="text-sm text-tride-text-muted">Real backend settings from dispatch_settings table</p>
                            </div>
                            <Button size="sm" onClick={saveDispatchSettings} disabled={savingSettings || !dispatchSettings}>
                                {savingSettings ? 'Saving...' : 'Save Config'}
                            </Button>
                        </div>

                        {!dispatchSettings ? (
                            <div className="text-tride-text-muted">Loading dispatch settings...</div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Max Search Radius Miles</label>
                                    <Input type="number" value={dispatchSettings.max_search_radius_miles ?? ''} onChange={(e) => updateDispatchSetting('max_search_radius_miles', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Accept Timeout Seconds</label>
                                    <Input type="number" value={dispatchSettings.accept_timeout_seconds ?? ''} onChange={(e) => updateDispatchSetting('accept_timeout_seconds', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Driver Batch Size</label>
                                    <Input type="number" value={dispatchSettings.driver_batch_size ?? ''} onChange={(e) => updateDispatchSetting('driver_batch_size', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Stale Order Minutes</label>
                                    <Input type="number" value={dispatchSettings.stale_order_minutes ?? ''} onChange={(e) => updateDispatchSetting('stale_order_minutes', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Distance Weight</label>
                                    <Input type="number" value={dispatchSettings.distance_weight ?? ''} onChange={(e) => updateDispatchSetting('distance_weight', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Rating Weight</label>
                                    <Input type="number" value={dispatchSettings.rating_weight ?? ''} onChange={(e) => updateDispatchSetting('rating_weight', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Vehicle Match Weight</label>
                                    <Input type="number" value={dispatchSettings.vehicle_match_weight ?? ''} onChange={(e) => updateDispatchSetting('vehicle_match_weight', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Acceptance Rate Weight</label>
                                    <Input type="number" value={dispatchSettings.acceptance_rate_weight ?? ''} onChange={(e) => updateDispatchSetting('acceptance_rate_weight', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Surge Multiplier</label>
                                    <Input type="number" value={dispatchSettings.surge_multiplier ?? ''} onChange={(e) => updateDispatchSetting('surge_multiplier', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Dispatch Mode</label>
                                    <select
                                        value={dispatchSettings.dispatch_mode ?? 'balanced'}
                                        onChange={(e) => updateDispatchSetting('dispatch_mode', e.target.value)}
                                        className="w-full mt-2 bg-tride-hover border border-tride-border rounded-xl px-3 py-2 text-sm text-tride-text"
                                    >
                                        <option value="nearest">Nearest Driver</option>
                                        <option value="balanced">Balanced Score</option>
                                        <option value="rating_priority">Rating Priority</option>
                                    </select>
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Max Retry Attempts</label>
                                    <Input type="number" value={dispatchSettings.max_retry_attempts ?? ''} onChange={(e) => updateDispatchSetting('max_retry_attempts', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl">
                                    <label className="text-sm font-medium text-tride-text">Driver Reject Cooldown Seconds</label>
                                    <Input type="number" value={dispatchSettings.driver_reject_cooldown_seconds ?? ''} onChange={(e) => updateDispatchSetting('driver_reject_cooldown_seconds', Number(e.target.value))} />
                                </div>

                                <div className="p-4 border border-tride-border rounded-xl flex items-center justify-between">
                                    <div>
                                        <p className="text-sm font-medium text-tride-text">Auto Dispatch Enabled</p>
                                        <p className="text-xs text-tride-text-muted">Allow the dispatch engine to assign orders automatically.</p>
                                    </div>
                                    <input
                                        type="checkbox"
                                        checked={Boolean(dispatchSettings.auto_dispatch_enabled)}
                                        onChange={(e) => updateDispatchSetting('auto_dispatch_enabled', e.target.checked)}
                                        className="w-5 h-5"
                                    />
                                </div>
                                ...
                                <div className="p-4 border border-tride-border rounded-xl flex items-center justify-between">
                                    <div>
                                        <p className="text-sm font-medium text-tride-text">Surge Enabled</p>
                                        <p className="text-xs text-tride-text-muted">Enable surge multiplier logic.</p>
                                    </div>
                                    <input
                                        type="checkbox"
                                        checked={Boolean(dispatchSettings.surge_enabled)}
                                        onChange={(e) => updateDispatchSetting('surge_enabled', e.target.checked)}
                                        className="w-5 h-5"
                                    />
                                </div>
                                ...
                                <div className="p-4 border border-red-500/30 bg-red-500/5 rounded-xl flex items-center justify-between">
                                    <div>
                                        <p className="text-sm font-medium text-red-400">Emergency Stop</p>
                                        <p className="text-xs text-tride-text-muted">Stop all auto-dispatch activity immediately.</p>
                                    </div>
                                    <input
                                        type="checkbox"
                                        checked={Boolean(dispatchSettings.emergency_stop)}
                                        onChange={(e) => updateDispatchSetting('emergency_stop', e.target.checked)}
                                        className="w-5 h-5"
                                    />
                                </div>
                            </div>
                        )}
                        <div className="bg-tride-card border border-tride-border rounded-3xl p-6 mt-6">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-lg font-semibold text-tride-text">Platform Commission Settings</h3>
                                    <p className="text-sm text-tride-text-muted">Controls T-Ride share and driver payout.</p>
                                </div>
                                <Button size="sm" onClick={saveCommissionSettings} disabled={savingCommissions || commissionSettings.length === 0}>
                                    {savingCommissions ? 'Saving...' : 'Save Commission'}
                                </Button>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {commissionSettings.map((item: any) => (
                                    <div key={item.service_type} className="p-4 border border-tride-border rounded-xl">
                                        <label className="text-sm font-medium text-tride-text capitalize">
                                            {item.service_type} Commission %
                                        </label>
                                        <Input
                                            type="number"
                                            value={item.commission_percent ?? ''}
                                            onChange={(e) => updateCommissionSetting(item.service_type, Number(e.target.value))}
                                        />
                                        <p className="text-xs text-tride-text-muted mt-2">
                                            Driver gets {100 - Number(item.commission_percent || 0)}%
                                        </p>
                                    </div>
                                ))}
                            </div>
                ...
                {activeTab === "Demand Heatmap" && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                >
                                    {dispatchZones.map((zone: any) => (
                                        <Circle
                                            ...
                                            options={{
                                                fillColor: zone.heat_level === 'high' ? '#ef4444' : zone.heat_level === 'medium' ? '#f59e0b' : '#22c55e',
                                                fillOpacity: zone.heat_level === 'high' ? 0.22 : zone.heat_level === 'medium' ? 0.16 : 0.10,
                                                strokeColor: zone.heat_level === 'high' ? '#ef4444' : zone.heat_level === 'medium' ? '#f59e0b' : '#22c55e',
                                                strokeOpacity: 0.70,
                                                strokeWeight: 2,
                                            }}
                        ...
                        <div className="bg-tride-card border border-tride-border rounded-3xl p-6">
                            ...
                            <div className="space-y-3">
                                {dispatchZones.length === 0 && (
                                    <div className="p-4 rounded-2xl border border-tride-border text-tride-text-muted">
                                        No dispatch zones found.
                                    </div>
                                )}
                                {dispatchZones.map((z: any) => (
                                    <div
                                        key={z.id}
                                    ...
                                    >
                                        <div className="flex justify-between items-start">
                                            <div>
                                                ...
                                                <p className="text-xs text-tride-text-muted">
                                                    {z.heat_level} demand · {z.pending_orders_count} orders · {z.drivers_count} drivers
                                                </p>
                                            </div>
                                            <span
                                        ...
                                        <p className="text-xs font-medium mt-1 text-orange-400">
                                            Radius: {z.radius_meters}m · Surge {z.surge_multiplier}x · Score {z.heat_score}
                                        </p>
                                    </div>
                                ))}
                            </div>
