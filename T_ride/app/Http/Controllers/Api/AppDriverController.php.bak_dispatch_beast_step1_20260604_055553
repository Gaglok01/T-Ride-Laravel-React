...
class AppDriverController extends Controller
{
    ...
    public function getStatus()
    {
        ...
        $canDrive = (bool) $driver->admin_override || (
    ...
    public function toggleOnline(Request $request)
    {
        ...
        $adminOverride = $driver->account_status === 'approved'
            && $driver->status === 'Active';

        $canDrive = $adminOverride || (
            $driver->account_status === 'approved'
            && $driver->status === 'Active'
        ...
        if ($request->is_online && !$canDrive) {
            ...
            return response()->json([
                ...
                'eligibility' => [
                    ...
                    'admin_override' => (bool) $driver->admin_override,
                ],
            ], 422);
        }
    ...
    {
        $zones = ServiceZone::where('status', 'active')
            ...
            ->map(function ($zone) {
                return [
                    ...
                    'demand_level' => $zone->demand_level ?? 'normal',
                    'surge_multiplier' => (float) $zone->surge_multiplier,
                    'price_multiplier' => (float) $zone->price_multiplier,
                ];
            })
            ->values();
        ...
        return response()->json([
            'status' => true,
            'message' => 'Driver heat map loaded',
            'data' => $zones,
        ]);
    }
    ...
    public function getRideRequests()
    {
        ...
        $driverUser = $driver->user ?? \App\Models\User::find($driver->user_id);
        $driverLat = $driverUser->lat ?? null;
        ...
        $requests = Ride::where('status', 'searching')
            ...
            ->map(function ($ride) use ($driverLat, $driverLng) {
                $tripDistanceMiles = $this->calculateDistance(
                    $ride->pickup_lat,
                    $ride->pickup_lng,
                    $ride->dropoff_lat,
                    $ride->dropoff_lng
                ...
                if ($driverLat !== null && $driverLng !== null) {
                    $pickupDistanceMiles = $this->calculateDistance(
                        $driverLat,
                        $driverLng,
                        $ride->pickup_lat,
                        $ride->pickup_lng
                ...
                $ride->distance_miles = round($tripDistanceMiles, 1);
            ...
            ->filter(function ($ride) use ($driverLat, $driverLng, $maxPickupMiles) {
                if ($driverLat === null || $driverLng === null) {
                    return false;
                }

                return $ride->pickup_distance_miles !== null
                    && $ride->pickup_distance_miles <= $maxPickupMiles;
            })
            ->values();
    ...
    {
        $validator = Validator::make($request->all(), [
            'status' => 'required|in:arrived,started,in_progress,completed,cancelled',
        ]);
        ...
        try {
            app(FirestoreRideSyncService::class)->sync($ride, $next);
        } catch (\Throwable $e) {
            \Log::error('Firestore status sync failed', ['ride_id' => $ride->id, 'status' => $next, 'error' => $e->getMessage()]);
        }
