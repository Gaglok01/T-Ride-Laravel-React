...
class AppDriverController extends Controller
{
    ...
    public function getRideRequests()
    {
        ...
        // In a real app, this would use websockets or filter by location
        // Here we just return rides with status 'searching' that match driver's vehicle type
        $requests = Ride::where('status', 'searching')
            ->where('vehicle_type_id', $driver->type_id)
            ->with(['rider'])
            ->latest()
            ->get();
