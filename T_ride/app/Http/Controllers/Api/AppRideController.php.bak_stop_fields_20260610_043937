...
class AppRideController extends Controller
{
    public function getEstimates(Request $request)
    {
        ...
        $surgeMultiplier = ($dispatchSettings->surge_enabled && $surgeZone)
            ? max((float) $dispatchSettings->surge_multiplier, (float) $surgeZone->surge_multiplier)
            : 1.00;
        ...
        foreach ($drivers as $driver) {
            ...
            if ($distanceMiles <= 8) {
                ...
                $availableDrivers[] = [
                    ...
                    'vehicle' => trim(($driver->vehicle_color ? $driver->vehicle_color . ' ' : '') . ($driver->vehicle_make ? $driver->vehicle_make . ' ' : '') . ($driver->vehicle_model ?? '')),
    ...
    public function requestRide(Request $request)
    {
        ...
        try {
            app(FirestoreRideSyncService::class)->sync($ride, 'searching');
        } catch (\Throwable $e) {
            \Log::error('Firestore searching sync failed', ['ride_id' => $ride->id, 'error' => $e->getMessage()]);
        }
        ...
        return response()->json([
            'status' => true,
            'message' => 'Ride requested, searching for driver...',
            'data' => $ride
        ]);
    }
    ...
    public function getActiveRide()
    {
        $ride = Ride::where('rider_id', Auth::id())
            ->whereIn('status', ['searching', 'assigned', 'accepted', 'arrived', 'in_progress'])
    ...
    private function findDispatchSurgeZone(float $lat, float $lng): ?DispatchZone
    {
        $zones = DispatchZone::where('is_active', true)->get();
        $matched = null;
        ...
        foreach ($zones as $zone) {
            ...
            if ($meters <= (float) $zone->radius_meters) {
                if (!$matched || (float) $zone->surge_multiplier > (float) $matched->surge_multiplier) {
                    $matched = $zone;
                }
            }
        }
